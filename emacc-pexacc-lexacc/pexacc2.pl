...
if ( scalar( @ARGV ) < 2 ) {
	...
	--input <--output file from emacc2.pl or equivalent> \\
	[--output <output file>]\n" );
}
...
sub runInParallel( $$$$ ) {
	...
	if ( $mach !~ /^${thishostname}/ ) {
		...
		pexacc2conf::portableForkAndDetach( "perl emacc-pexacc-lexacc\\pdataworker.pl ${infile}" );
	}
}
